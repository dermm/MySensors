...
#define REQUEST_REPLY_DATA_BASE_SIZE 60
#define ALLOW_TIME_ERROR_IN_SYNC_MESSAGE true // false=Validate sync messages against own RTC time
...
void espNowFloodingMesh_send(uint8_t* msg, int size, int ttl, int tryCount)  {
...
bool espNowFloodingMesh_sendAndWaitReply(uint8_t* msg, int size, int ttl, int tryCount, void (*f)(const uint8_t *, int), int timeoutMs, int expectedCountOfReplies){
  ...
  for(int i=0;i<tryCount;i++) {
    ...
    espNowFloodingMesh_sendAndHandleReply(msg, size, ttl, [](const uint8_t *data, int len){
      ...
      replyCnt++;
      //Serial.print("Rec reply ");Serial.println(counter);
    });
    //Serial.print("Send request ");Serial.print(counter);Serial.print(" with "); Serial.println(ret);
